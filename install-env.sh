...
apt-get update
apt-get install cmake libjpeg62 python-pygame python3-pip pymol python3-scipy python3-numpy python-scipy python-numpy idle-python3.4 python-igraph virtualbox vim git openbabel muscle jalview autodock autogrid autodock-vina vlc icedtea-7-plugin python3-matplotlib python-matplotlib libjpeg62 exfat-utils flashplugin-installer exfat-fuse gimp tiled tupi blender plink emboss octave octave-info grace r-base-core r-base-dev gfortran gromacs libx11-dev liblzma-dev csh libxml2 svn figtree labyrinth dia pdb2pqr libxml2-dev libopenblas-dev


#Install MODELLER
cd ${sourcedir}
wget "https://salilab.org/modeller/9.16/modeller_9.16-1_amd64.deb"
dpkg -i modeller_9.16-1_amd64.deb 
sed -i 's/xxx/${modeller_key}/' /usr/lib/modeller9.16/modlib/modeller/config.py

#Python libraries
pip3 install biopython python-igraph cython mdtraj tensorflow Theano keras
pip install cython mdtraj tensorflow Theano keras

#R libraries
apt-get install r-cran-rgl r-cran-ggplot2 r-cran-nnet r-cran-amore r-cran-caret r-cran-seqinr
